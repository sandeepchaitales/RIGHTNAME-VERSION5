...
async def dynamic_brand_search(brand_name: str, category: str = "") -> dict:
    ...
    try:
        ...
        llm = LlmChat(EMERGENT_KEY, "openai", "gpt-4o")  # Stable model for brand recognition
...
async def evaluate_brands(request: BrandEvaluationRequest):
    ...
    if LlmChat and EMERGENT_KEY:
        # Try Claude first since OpenAI is having 502 issues
        models_to_try = [
            ("anthropic", "claude-sonnet-4-20250514"),  # Primary - Claude Sonnet (most reliable)
            ("openai", "gpt-4o"),                       # Fallback 1 - GPT-4o
            ("openai", "gpt-4.1"),                      # Fallback 2 - GPT-4.1
        ]
    else:
            ...
            try:
                ...
                if any(x in error_msg for x in ["502", "BadGateway", "ServiceUnavailable", "Expecting", "JSON", "validation error", "control character"]):
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logging.warning(f"All {max_retries} retries exhausted for {model_provider}/{model_name}")
                        break  # Exit retry loop, try next model
                
                logging.error(f"LLM Error with {model_provider}/{model_name}: {error_msg[:200]}")
